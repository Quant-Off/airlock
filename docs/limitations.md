...
Landlock과 Seatbelt가 EPERM을 낸 시점에 브로커는 개입하지 않습니다. gap으로 노출됩니다 (`landlock.rs:835-837`).
...
### 2.5 exec 규칙은 경계가 아니라 tripwire다

argv 수준 매칭은 원리적으로 우회 가능합니다. `sh -c 'r''m -rf /'`, base64 디코드, 인터프리터 경유가 전부 회피 경로입니다. `docs/policy-dsl.md` 7.1절이 이를 명시합니다.

실제 방어는 파일 규칙과 egress 규칙에서 나옵니다. 다만 egress 강제가 아직 포트까지뿐이므로(3장), 지금 시점의 실질 경계는 **파일 규칙 하나**입니다.
...
macOS 세션의 체인에는 제네시스, 최상위 exec 판정 하나, `session_end` 셋만 남습니다. 자식 exec, 아웃바운드 연결, 파일 접근은 하나도 기록되지 않습니다. `--mediate full`은 받아들여지고 아무 일도 하지 않으며, 그 사실이 배너와 제네시스에 남습니다 (`session.rs:87-94`).

### 2.7 Landlock은 `[defaults]`를 반영하지 않는다

허용 계획에서 빼는 것은 **실제로 매칭된 규칙이 막을 때뿐**이고, `[defaults]`로 떨어진 경로는 빼지 않습니다 (`landlock.rs:315`). 근거는 코드 주석에 있습니다 (`landlock.rs:293-299`). `[defaults].file = "ask"`인 베이스라인에서 기본값까지 반영하면 아무것도 허용되지 않아 프로세스가 exec조차 못 하기 때문입니다.

의도된 선택이지만 결과는 비대칭입니다. macOS 프로파일은 진짜 `(deny default)`인데 (`profile.rs:133`) Linux는 그렇지 않습니다. **같은 정책 파일이 두 OS에서 다른 커널 자세를 만들고, 이 차이는 gap 목록에 나오지 않습니다.**
...
`WALK_BUDGET = 200_000`이며 루트마다 새로 줍니다 (`landlock.rs:43`, `280-291`). 예산이 0이 되면 항목 루프가 끊기고 `truncated`가 서고 (`landlock.rs:348-352`), 그 뒤 디렉토리는 `ReadDir` 권한만 받습니다 (`landlock.rs:401`, `421`).
...
gap 보고는 각각 5개까지만 보여 줍니다 (`landlock.rs:489`, `495`). 4천 개가 잘렸어도 사용자는 5개만 봅니다.
...
계획은 fork 이전 부모에서 세우고 (`landlock.rs:794`), 적용은 `pre_exec`에서 합니다 (`landlock.rs:816-817`).
...
`kind.is_symlink()`면 개수만 세고 건너뜁니다 (`landlock.rs:359-366`). 어떤 링크가 빠졌는지는 보고하지 않습니다 (`landlock.rs:501-507`).
...
- 사용자 티어여야 합니다. 베이스라인 티어는 보지 않습니다 (`landlock.rs:467`).
- glob이 없어야 합니다. `*`나 `?`가 있으면 건너뜁니다 (`landlock.rs:479-481`).
- 이미 존재해야 합니다 (`landlock.rs:483`).

곧 `~/data/**` 같은 allow 규칙은 커널 수준에서 아무것도 열지 않고, 아직 없는 경로는 나중에도 만들 수 없습니다. **셋 다 gap으로 보고되지 않습니다.**

### 3.6 네트워크는 TCP connect 포트뿐이고 호스트는 버려진다

계획의 키는 포트뿐이며 (`landlock.rs:560-563`), 호스트는 gap 문자열을 만들기 위해서만 쓰입니다 (`landlock.rs:569-571`). 실제 규칙은 `NetPort::new(port, AccessNet::ConnectTcp)`입니다 (`landlock.rs:762`).
...
- **`port` 없는 egress allow 규칙 하나가 네트워크 강제를 통째로 끕니다** (`landlock.rs:564-567`, 가드는 `723`과 `759`). gap은 나오지만 강제는 이미 사라진 뒤입니다.
- **`deny` egress 규칙은 계획이 아예 보지 않습니다** (`landlock.rs:557-559`). `deny host="169.254.169.254"`는 커널 효과가 0이고, macOS와 달리 "옮기지 못했다"는 보고조차 없습니다.

### 3.7 TCP bind는 항상 막히는데 gap이 없다

`handle_access`는 `BindTcp`와 `ConnectTcp`를 함께 다루지만 (`landlock.rs:725`) 규칙은 `ConnectTcp`만 추가합니다 (`landlock.rs:759-765`).

네트워크 제한이 켜지면 자식은 어떤 TCP 포트도 bind할 수 없습니다. 개발 서버, 언어 서버, 테스트 하네스가 이 때문에 실패하고 어떤 gap 문자열도 bind를 언급하지 않습니다.

### 3.8 UDP는 전혀 강제되지 않는다

Landlock ABI v10부터이며 crate가 아직 노출하지 않습니다. gap으로 밝힙니다 (`landlock.rs:845-849`). DNS 기반 반출과 QUIC/HTTP-3 아웃바운드는 커널 층을 통째로 비껴갑니다.

### 3.9 ABI 강등 중 일부가 gap에 나오지 않는다

`detect_abi()`가 커널 버전으로 ABI를 추정하고 8을 상한으로 잡습니다 (`landlock.rs:645-673`).

- ABI < 4: 아웃바운드 제한 없음. **gap 있음** (`landlock.rs:839-844`).
- ABI < 6: 스코프 제한이 없어 자식이 **브로커와 감독 스레드에 시그널을 보낼 수 있고 추상 유닉스 소켓(ssh-agent, dbus)에 연결할 수 있습니다.** 근거는 주석에 있으나 (`landlock.rs:729-731`) **gap 문자열이 없습니다.**
- ABI < 3: `Truncate` 미중재. gap 없음.
- ABI < 5: `IoctlDev` 미중재. gap 없음.

### 3.10 부분 강제가 원시 stderr 경고 하나로 통과한다

`FullyEnforced`가 아니면 `warn_partial()`을 부르고 계속 진행합니다 (`landlock.rs:823-825`). 그 함수는 고정 문자열을 `libc::write(2, ...)`로 씁니다 (`landlock.rs:709-715`).

경고가 **자식의** fd 2로 나가므로 stderr를 리다이렉트하거나 에이전트가 삼키면 사라지고, 감사 체인에는 절대 남지 않습니다. `NotEnforced`만 중단시킵니다 (`landlock.rs:818-822`).

### 3.11 시스템 읽기 루트가 8개 고정이다

`/usr`, `/bin`, `/sbin`, `/lib`, `/lib64`, `/opt`, `/etc`, `/proc/self`뿐입니다 (`landlock.rs:45-54`).

`/var`, `/run`, `/sys`, `/nix`, `/snap`과 `/proc/self` 밖의 `/proc`이 전부 닿지 않습니다. `/proc/cpuinfo`나 `/sys/devices/system/cpu`를 읽는 툴체인이 실패하고, **NixOS와 Snap 패키지 바이너리는 아예 실행되지 않습니다.** 쓰기 가능 장치 노드도 5개 고정입니다 (`landlock.rs:61-67`).

### 3.12 exec 규칙은 Landlock으로 강제할 수 없다

gap으로 밝힙니다 (`landlock.rs:520-545`). Linux에서 모든 `kind = "exec"` deny/ask는 전적으로 중계 층에 의존하며, 중계는 TOCTOU를 부담하고 끌 수도 있습니다. 같은 규칙이 macOS에서는 `(deny process-exec* ...)`로 커널까지 내려갑니다 (`profile.rs:292-297`).
...
| 정책               | 상태              | 근거                              |
|------------------|-----------------|---------------------------------|
| exec argv 조건     | **불가능**         | `profile.rs:314-317`            |
| 포트/호스트 단위 egress | 프록시 층에서만 가능     | `profile.rs:203-208`            |
| `ask` (파일)       | deny로 강등        | `profile.rs:221`, `391-393`     |
| `ask` (exec)     | **프로파일에 넣지 않음** | `profile.rs:285-288`, `395-398` |

exec `ask`를 넣지 않는 이유는 사람이 승인한 실행이 커널에서 막혀 어떤 방법으로도 진행할 수 없게 되기 때문입니다. 다만 macOS에는 중계 층도 없으므로(2.6), `danger-rm`/`sudo-exec`/`pipe-curl-to-shell` 같은 규칙은 **강제되지도 기록되지도 않습니다.**
...
`forms_of`는 비ASCII일 때만 원본 + NFC + NFD를 냅니다 (`sbpl.rs:124-138`). allow 규칙은 일부러 한 형태만 냅니다 (`profile.rs:364-370`).
...
`denied_overrides`로 모아 gap으로 보고합니다 (`seatbelt.rs:122-129`, `207-212`). 근거는 `docs/policy-dsl.md` 3.2절에 있습니다. SBPL이 "이 forbid에서 이 부분집합만 제외"라는 교집합을 표현하지 못하기 때문입니다.

곧 정당하게 완화한 사용자 규칙이 macOS에서는 실제로 동작하지 않습니다. fail-closed 방향이라 안전하지만, 정책 층과 커널의 결론이 갈립니다.

### 4.5 기본 열림 항목들

- `(allow process-exec*)` (`profile.rs:138`): 명시 deny가 없으면 모든 exec이 허용됩니다.
- `(allow mach-lookup)` (`profile.rs:145`): 클립보드 두 개만 막고 (`profile.rs:147-148`) 나머지 XPC/Mach 서비스는 전부 닿습니다. XPC로 오가는 것은 정책 모델 밖이자 감사 로그 밖입니다 (gap: `seatbelt.rs:190-192`).
- `(allow file-read-metadata)` (`profile.rs:155`): 시스템 전체 경로의 존재/크기/mtime을 읽을 수 있습니다.

### 4.6 `(with no-report)`가 OS 자체 위반 로그를 끈다

`(deny file-write* (with no-report))` (`profile.rs:134`).

거부된 쓰기는 시스템 샌드박스 위반 리포트를 남기지 않습니다. 중계 층 부재(2.6)와 gap(`seatbelt.rs:187-189`)이 겹쳐, **막힌 쓰기는 어디에도 흔적을 남기지 않습니다.**

### 4.7 deprecated API 위에 서 있고 실패 이유를 버린다

`sandbox_init_with_parameters`를 직접 선언해 호출합니다 (`seatbelt.rs:18-27`, `89-91`). 10.8부터 deprecated입니다. 실패는 0이 아닌 반환값으로만 드러나고 오류 버퍼는 버립니다 (`seatbelt.rs:98-103`). 호출자는 고정 문자열을 받고 커널이 준 이유는 못 받습니다.

### 4.8 `sandbox-exec` 교차 검증 경로는 프로파일 전문을 `ps`에 노출한다

`wrapped.arg("-p").arg(text)` (`seatbelt.rs:172-173`). 같은 머신의 어떤 프로세스든 프로세스 테이블에서 정책 전문을 읽습니다. gap으로 밝히고 (`seatbelt.rs:213-219`) CLI는 이 경로를 고르지 않지만 (`cmd_run.rs:310-321`), `SeatbeltEnforcer`의 공개 API로는 남아 있습니다.

### 4.9 basename 규칙이 glob 메타문자를 이스케이프하지 않는다

`escape_regex_char`는 `. + ( ) [ ] { } ^ $ | \`를 이스케이프하지만 `*`와 `?`는 하지 않습니다 (`sbpl.rs:52-60`, `156-161`).

`program = "a?c"`는 `^.*/a?c$`로 나가고 여기서 `?`는 정규식 수량자라 `a`가 선택적이 됩니다. **방출된 deny가 정책이 말하는 것보다 넓어집니다.**

### 4.10 프로파일 크기 상한 검사가 없다

`generate`는 패턴 × 정규화 변형마다 한 줄씩 무한히 덧붙입니다 (`profile.rs:378-385`). 사전 검사는 내부 NUL 바이트뿐입니다 (`seatbelt.rs:130-133`). 큰 정책은 커널의 컴파일 프로파일 상한을 넘길 수 있고, 그때의 실패는 진단 가능한 정책 문제가 아니라 spawn 시점의 일반 오류로 보입니다.

### 4.11 OS 버전 매트릭스가 러너 하나뿐이다

CI는 `macos-15` 하나만 돌립니다 (`.github/workflows/ci.yml:19`). `docs/design.md` 10.3절이 지적한 대로 OS 마이너 버전이 올라가면 이전에 참조하지 않던 리소스를 셸이 읽어 거부가 생기는 회귀가 실제로 관측됩니다. 그 회귀를 잡을 매트릭스가 지금은 없습니다.
...
- **`**/.envrc`** (direnv. 시크릿이자 셸 코드 실행인데 `**/.env`/`**/.env.*` 어느 쪽에도 매칭하지 않습니다)
- `**/*.pem`, `**/*.key`, `**/*.p12`
...
| 상수                   | 값                         | 위치                                      |
|----------------------|---------------------------|-----------------------------------------|
| Landlock 순회 예산 (루트당) | `200_000`                 | `broker/landlock.rs:43`                 |
| 시스템 읽기 루트            | 8개                        | `broker/landlock.rs:45-54`              |
| 쓰기 가능 장치 노드 (Linux)  | 5개                        | `broker/landlock.rs:61-67`              |
| gap 보고 상한            | `take(5)` × 2             | `broker/landlock.rs:489`, `495`         |
| Landlock ABI 상한      | `V8`                      | `broker/landlock.rs:670`                |
...
| 시스템 읽기 서브패스 (macOS)  | 14개                       | `broker/profile.rs:15-30`               |
| 쓰기 가능 장치 노드 (macOS)  | 8개                        | `broker/profile.rs:38-47`               |
...
**상한이 없는 것**도 함께 적어 둡니다. 규칙 수, 규칙당 패턴 수, 정책 파일 크기, 경고 개수 (`policy/engine.rs:370-375`), SBPL 프로파일 크기 (`broker/profile.rs:378-385`), 감사 체인 한 줄의 길이 (`audit/verify.rs:253-261`), 감사 로그 총 크기.

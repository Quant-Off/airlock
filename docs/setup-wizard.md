...
6. 쓴 파일의 다이제스트를 정책 신뢰 기록에 자동으로 남긴다 (`docs/design.md` 9.6). 사용자가 방금 생성에 동의했으므로 첫 `airlock run` 이 다시 묻지 않는다. 기록 실패는 경고와 `airlock policy trust` 안내로만 남는다
7. 요약과 함께 바로 실행할 `airlock run` 명령을 안내

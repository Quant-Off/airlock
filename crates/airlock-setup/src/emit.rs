...
mod tests {
    ...
    fn substitutes_workspace_rule_path() {
        for id in ["claude-code", "strict"] {
            let rendered = render(preset(id).source, Some("~/proj/demo")).expect(id);
    ...
    fn keeps_developer_preset_unchanged() {
        let source = preset("developer").source;
    ...
    fn preserves_comments_and_stays_loadable() {
        let rendered = render(preset("claude-code").source, Some("/work/demo")).expect("render");
    ...
    fn trims_trailing_slash() {
        let rendered = render(preset("strict").source, Some("/work/demo/")).expect("render");

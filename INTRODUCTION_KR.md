...
| exec 화이트리스트 (커널 강제)                  | **구현됨** | 양 플랫폼. `[defaults].exec`가 `allow`가 아닐 때만 걸림           |
| 평문 아웃바운드 차단                           | **구현됨** | `--egress-proxy` 위에서만. 중계 층은 프로토콜을 보지 못함         |
| 세션 상위 앵커 체인                            | **구현됨** | `--anchor-dir`로 분리해야 재계산 탐지력이 생김                    |
| 매일 점검 보고와 책임자 확인                   | **구현됨** | `audit report`, `audit ack`. 판단 불가를 통과로 보고하지 않음     |
| 목적지별 반출량 기록과 총량 한도               | **구현됨** | 프록시 전용. 한도는 다음 연결부터 적용됨                          |
| egress DLP (내용 기반 시크릿 패턴 차단)        |   미구현   | 프록시가 TLS를 종단하지 않음. 메타데이터만 기록됨                 |
| 감사 로그 저장 시 암호화                       |   미구현   | 0600 평문. 봉투는 체인 밖이라는 경로만 규격에 예약                |
| MCP 프록시 층                                  |   미구현   |                                                                   |
...
| exec 경로/파일 이름          | **커널 강제** (화이트리스트)                  | **커널 강제** (화이트리스트)           |
...
| 평문 아웃바운드 차단         | `--egress-proxy` 위에서만                     | `--egress-proxy` 위에서만              |
| 목적지별 반출량 기록         | `--egress-proxy` 전용                         | `--egress-proxy` 전용                  |
| 자식 프로세스 exec 기록      | `--mediate`로 켜짐 (기본값)                   | **기록 안 됨.** 중계 기구가 없음       |
| 자식 프로세스 아웃바운드 기록 | `--mediate` 또는 `--egress-proxy`로 켜짐      | `--egress-proxy`에서만 기록됨          |
| 자식 프로세스 파일 열기 기록 | `--mediate full`로 켜짐                       | **기록 안 됨.** 중계 기구가 없음       |

곧 macOS에서는 그 아래 자식들이 무엇을 실행하고 어떤 파일을 열었는지가 감사에 남지 않습니다. 어디로 연결했는지는 예외입니다. egress 프록시는 커널 중계 기구가 아니라 브로커 안의 판정 지점이라, `--egress-proxy`에서는 중계 수준이 `off`여도 자식의 아웃바운드가 판정되고 기록됩니다. `--mediate` 값은 macOS에서 적용되지 않으며, 그 사실이 배너와 감사 로그 제네시스 양쪽에 기록됩니다.
...
감사 로그는 엔트리 내용 변조, 순서 바꾸기, 중간 삭제, 해시를 다시 봉인한 삽입, 꼬리 잘라내기, 다른 세션 엔트리 이식을 탐지합니다. 엔트리에 포맷 버전이 해시와 함께 묶여 있어 **옛 포맷 체인과 위조된 체인이 구분**되며, 버전만 낮춰 옛 검증 규칙을 끌어오는 경로도 없습니다.

**체인 전체를 처음부터 재계산할 수 있는 공격자**는 세션 로그 하나만으로는 탐지되지 않습니다. 그래서 세션마다 최종 head를 상위 앵커 체인(`anchors.jsonl`)에 남깁니다. `--anchor-dir`로 그 파일을 **다른 볼륨이나 원격 마운트로 분리**하면 세션 통째 삭제와 체인 재계산이 탐지됩니다. 감사 루트 안에 그대로 두면 체인을 재계산할 수 있는 주체가 앵커도 같은 비용으로 재계산하므로 탐지력이 없으며, `airlock run`의 배너가 그 사실을 매번 출력합니다.

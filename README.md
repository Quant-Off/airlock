...
Airlock에서 신뢰 가능한 컴퓨팅 기반(Trusted Computing Base, TCB)은 `broker`(브로커) 크레이트만이며, 에이전트나 툴, MCP 서버, LLM은 전부 신뢰 경계 밖에 있습니다.
...
정책(policy) 파일이 없으면 내장 베이스라인만이 적용됩니다. 현재 디렉토리의 `airlock.toml` 또는 `.airlock.toml`, 없으면 `~/.config/airlock/policy.toml`을 순서대로 찾습니다. 상위 디렉토리로 거슬러 올라가지는 않습니다. 예제는 `examples/policy/`에 있습니다.

```bash
$ cp examples/policy/strict.toml airlock.toml
```

Linux에서는 자식 프로세스가 부르는 `execve`와 `connect`를 브로커로 중계해 감사에 남깁니다. 이것이 없으면 `airlock run`이 직접 띄운 프로세스 하나만 기록되고 그 아래에서 벌어지는 일은 보이지 않습니다.
...
| macOS 커널 강제 (Seatbelt)           | **구현됨** | 파일 규칙이 커널에서 강제됨       |
| `/dev/tty` 인라인 ask 승인            | **구현됨** |                       |
| Linux 커널 강제 (Landlock)           | **구현됨** | 파일 규칙과 TCP 포트가 커널에서 강제됨 |
...
```
airlock 0.1.0
  정책     baseline (22 규칙, 다이제스트 af8569a65f45)
  강제     seatbelt (sandbox_init_with_parameters)
  감사     ~/.local/share/airlock/sessions/1785073894508695000-38871
  한계     호스트 단위 egress 정책은 Seatbelt로 강제되지 않는다. 프록시 층이 필요하다
  한계     Seatbelt는 사람 승인을 표현할 수 없으므로 ask 규칙은 프로파일에서 deny로 내려간다
```
...
fmt, clippy(`-D warnings`), 전체 테스트, 라이브러리 코드 unwrap 및 expect 금지, 정책 프리셋 로드를 한 번에 확인합니다. 아직 CI는 없고 이 스크립트가 유일한 검증 경로입니다. unwrap을 금지하는 이유는 브로커가 TCB이기 때문이며, 삼켜진 실패 경로 하나가 곧 강제 층의 구멍이 됩니다.
